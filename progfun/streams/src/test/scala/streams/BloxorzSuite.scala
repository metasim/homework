package streams

import org.scalatest.FunSuite

import org.junit.runner.RunWith
import org.scalatest.junit.JUnitRunner

import Bloxorz._

@RunWith(classOf[JUnitRunner])
class BloxorzSuite extends FunSuite {

    trait SolutionChecker extends GameDef with Solver with StringParserTerrain {
        /**
         * This method applies a list of moves `ls` to the block at position
         * `startPos`. This can be used to verify if a certain list of moves
         * is a valid solution, i.e. leads to the goal.
         */
        def solve(ls: List[Move]): Block =
            ls.foldLeft(startBlock) {
                case (block, move) => move match {
                    case Left => block.left
                    case Right => block.right
                    case Up => block.up
                    case Down => block.down
                }
            }
    }

    trait Level1 extends SolutionChecker {
        /* terrain for level 1*/

        val level =
    """ooo-------
      |oSoooo----
      |ooooooooo-
      |-ooooooooo
      |-----ooToo
      |------ooo-""".stripMargin

        val optsolution = List(Right, Right, Down, Right, Right, Right, Down)
    }

    trait LevelNone extends SolutionChecker {
        /* terrain for level 1*/

        val level =
    """ooo-------
      |oSoooo----
      |ooooooooo-
      |-ooooooooo
      |-----ooooo
      |T-----ooo-""".stripMargin

        val optsolution = List()
    }
        
    test("terrain function level 1") {
        new Level1 {
            assert(terrain(Pos(0, 0)), "0,0")
            assert(!terrain(Pos(4, 11)), "4,11")
        }
    }

    test("findChar level 1") {
        new Level1 {
            assert(startPos == Pos(1, 1))
        }
    }

    test("is standing") {
        new Level1 {
            assert(Block(Pos(1, 2), Pos(1, 2)).isLegal)
            assert(Block(Pos(1, 2), Pos(1, 2)).isStanding)
            assert(Block(Pos(1, 2), Pos(1, 3)).isLegal)
            assert(!Block(Pos(1, 2), Pos(1, 3)).isStanding)
        }
    }

    test("start position") {
        new Level1 {
            assert(startBlock == Block(Pos(1, 1), Pos(1, 1)))
        }
    }

    test("neighbors") {
        new Level1 {
         
            val n = startBlock.neighbors
            assert(n === List(
                (Block(Pos(1, -1), Pos(1, 0)), Left),                
                (Block(Pos(1, 2), Pos(1, 3)), Right),
                (Block(Pos(-1, 1), Pos(0, 1)), Up),
                (Block(Pos(2, 1), Pos(3, 1)), Down)
            ))
            
            val l = startBlock.legalNeighbors
            assert(l === List(
                (Block(Pos(1, 2), Pos(1, 3)), Right),
                (Block(Pos(2, 1), Pos(3, 1)), Down)
            ))

        }
    }

    test("understanding done") {
        new Level1 {
            assert(done(Block(Pos(4,7), Pos(4,7))))
        }
    }
    
    test("neighbors have history") {
        new Level1 {
            val history = neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up))
           
            assert(history.toSet === Set(
                (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
                (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))))
        }
    }
    
    test("don't wear out your welcome with the neighbors") {
        new Level1 {
            val neighbors = neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up))
            val newGuys = newNeighborsOnly(neighbors, Set(Block(Pos(1,2),Pos(1,3))))
            assert(newGuys.size === 1)
            assert(newGuys(0)._1 === Block(Pos(2,1),Pos(3,1)))
            
            val newGuys2 = newNeighborsOnly(neighbors, Set(Block(Pos(2,1),Pos(3,1))))
            assert(newGuys2.size === 1)            
            assert(newGuys2(0)._1 === Block(Pos(1,2),Pos(1,3)))
            
        }
    }
    

    
    test("optimal solution for level 1") {
        new Level1 {
            assert(solve(solution) == Block(goal, goal))
        }
    }

    test("optimal solution length for level 1") {
        new Level1 {
            assert(solution.length == optsolution.length)
        }
    }
    
    test("no solution") {
        new LevelNone {
            assert(solution.length == 0)
        }
    }
}
